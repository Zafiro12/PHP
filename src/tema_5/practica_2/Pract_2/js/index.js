...
function comprobar_editado(nombre_original) {
    $("#error_cod").html("");
    $("#error_nombre_corto").html("");
    ...
    $.ajax({
        ...
        .done(function (data) {
            if (data.mensaje_error) {
                ...
                $.ajax({
                    url: encodeURI(DIR_SERV + "/familias"),
                    type: "GET",
                    dataType: "json",
                    data: { api_session: localStorage.api_session },
                })
                    .done(function (data2) {
                        if (data2.mensaje_error) {
                            ...
                            html_output +=
                                "<form onsubmit='event.preventDefault();seguridad(comprobar_editado, \"" +
                                data.producto["nombre_corto"] +
                                "\");'>";
                            html_output +=
                                "<p><label for='cod'>Código: </label><input type='text' id='cod' value='" +
                                data.producto["cod"] +
                                "' disabled/><span id='error_cod'></span></p>";
                            html_output +=
